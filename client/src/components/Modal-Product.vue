<template>
  <Teleport to="body">
    <div 
      ...
      class="fixed inset-0  flex items-center justify-center bg-gray-900 bg-opacity-50 z-50 max-h-100 overflow-hidden"
    >
      <div class="bg-white p-2 sm:p-4 md:p-6 rounded-lg shadow-lg  max-w-[800px]">
        <div v-if="selectedProduct" class="flex flex-col sm:flex-row gap-4  ">
          <div class="flex flex-col sm:w-1/2 relative">
            <img :src="selectedProduct.images" alt="Product Image" class="w-full h-auto rounded-lg shadow-md max-h-[320px] max-w-[320px]" />
            <div v-if="selectedProduct.discount > 0" class="absolute top-0 right-5 animate-pulse bg-gradient-to-r from-red-500 to-yellow-500 text-white p-1 rounded-bl-lg">
              {{ selectedProduct.discount }}% OFF
            </div>
            <div class="flex items-center justify-center mt-2 sm:mt-4 w-[310px] ">
          ...
          <div class="flex flex-col sm:w-1/2">
            <h1 class="text-md sm:text-xl md:text-2xl font-semibold">{{ selectedProduct.name }}</h1>
            <div class="flex items-center gap-1 sm:gap-2 mt-1 sm:mt-2 border-b pb-1 sm:pb-2">
              <a-rate :value="getstar" disabled allowHalf />
              <span class="text-xs sm:text-sm md:text-md text-red-600 pl-2 sm:pl-3">({{ getstar }} đánh giá)</span>
            </div>
            <span v-if="selectedProduct.discount > 0" class="text-xl  text-red-600 mt-1 sm:mt-2 border-b pb-1 sm:pb-2">
              Giá: <s>{{ selectedProduct.price }}$</s> {{ selectedProduct.discountedPrice }}$
            </span>
            <span v-else class="text-xl text-red-600 mt-1 sm:mt-2 border-b pb-1 sm:pb-2">
              Giá: {{ selectedProduct.price }}$
            </span>
            <p class="font-bold text-sm  ">{{ selectedProduct.title }}</p>
            <p class="text-xs ">Thể loại sách: {{ selectedProduct.title }}</p>
            <p class="text-xs ">Tác giả: {{ selectedProduct?.author?.name }}</p>
            <p class="text-xs ">Nhà xuất bản: {{ selectedProduct?.publisher.title }}</p>
            <p class="text-xs ">Ngày phát hành: {{ formatDate(selectedProduct.createdAt) }}</p>
            <div class="flex gap-1 mt-1 sm:mt-2 grid-flow-row overflow-hidden">
              <span class="text-xs sm:text-sm md:text-md text-red-600">Số lượng: {{ selectedProduct.quantity }}</span>
              <span class="text-xs sm:text-sm md:text-md text-green-600">đã bán: {{ selectedProduct.sold }}</span>
              <span class="text-xs sm:text-sm md:text-md text-red-600">Tình trạng: {{ selectedProduct.quantity > 0 ? 'Còn hàng' : 'Hết hàng' }}</span>
            </div>
