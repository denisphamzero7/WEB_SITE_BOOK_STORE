...
const actions = {
    ...
    async updateBlog({ commit }, { bid, data }) {
        try {
            const response = await BlogService.updateblogs(bid, data); // Corrected
            console.log(response.updateblog);
            commit('SET_BLOG', response.updateblog);
        } catch (error) {
            console.error('Error updating blog:', error); // Updated error message
        }
    },
